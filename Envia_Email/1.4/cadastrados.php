...
<form id="theForm" action="cadastrados.php?filtro" method="POST">		
...
<form id="theForm2" action="cadastrados.php?alterastatus" method="POST">
	...
	<input id="acao_input" name="acao_input" type="hidden" value="">
			...
			<table class="table table-striped table-bordered table-hover table-custom">
					...
					$sql = mysql_query("select id, assunto, status from scop_envia_email order by assunto ASC");
						while($exibe = mysql_fetch_assoc($sql)){
							...
							if($filtro == "todos" || $filtro == ""){							
								echo "<tr>";
									...
									//echo "<td><center><a class=\"excluir\" href=\"cadastrados.php?acao=excluir&id=".$exibe["id"]."\" onclick=\"verf('".$exibe['assunto']."')\"><span class=\"glyphicon glyphicon-trash\" style=\"color: black; padding-top:3px;\"></span></a></center></td>";
								echo "</tr>";
							} else {
